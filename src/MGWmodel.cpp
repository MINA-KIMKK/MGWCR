...
// [[Rcpp::export]]
List gw_reg_all(mat x, vec y, mat dp, vec di,bool rp_given, mat rp, bool dm_given, mat dmat, bool hatmatrix, 
                double p, double theta, bool longlat, 
                double bw, int kernel, bool adaptive,
                int ngroup=1, int igroup=0) {
  ...
  if (hatmatrix) {
    ...
    for (int i = iStart; i < iEnd; i++) {
      ...
      vec w = gw_weight(d, bw, kernel, adaptive);
      vec wd = w % di;
      mat xtw = trans(x) * diagmat(wd);
      mat xtwx = xtw * x;
      mat xtwy = trans(x) * diagmat(wd) * y;
      mat xtwx_inv = inv(xtwx);
      betas.row(i) = trans(xtwx_inv * xtwy);
      
      // hatmatrix
      mat ci = xtwx_inv * xtw;
      rowvec si = x.row(i) * ci;
...
// [[Rcpp::export]]
double rss(vec y, mat X, mat beta, vec di)
{
  vec r = ehat(y, X, beta);
  return sum(di % (r % r));
}
...
// [[Rcpp::export]]
double AICc(vec y, mat x, mat beta, mat S, vec di)
{
  double ss = rss(y, x, beta, di);
  vec s_hat = trhat2(S);
  int n = S.n_rows;
...
//Could be dropped
// [[Rcpp::export]]
double AICc1(vec y, mat x, mat beta, vec s_hat, vec di)
{
  double ss = rss(y, x, beta, di);
  // vec s_hat = trhat2(S);
  int n = x.n_rows;
...
// [[Rcpp::export]]
vec AICc_rss(vec y, mat x, mat beta, mat S, vec di)
{
  vec result(3);
  double ss = rss(y, x, beta, di);
  result[0] = ss;
...
// [[Rcpp::export]]
vec AICc_rss1(vec y, mat x, mat beta, vec s_hat, vec di)
{
  vec result(3);
  double ss = rss(y, x, beta, di);
  result[0] = ss;
...
//BIC calculation
// [[Rcpp::export]]
double gw_BIC(vec y, mat x, mat beta, vec s_hat, vec di)
{
  double ss = rss(y, x, beta, di);
