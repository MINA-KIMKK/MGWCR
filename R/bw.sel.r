...
##Author: Binbin Lu
gwr.aic<-function(bw, X, Y, di, kernel="bisquare",adaptive=FALSE, dp.locat, p=2, theta=0, longlat=F,dMat, verbose=T,parallel.method=F,parallel.arg=NULL)
{
  dp.n<-length(dp.locat[,1])
  var.n <- ncol(X)
  #########Distance matrix is given or not
  ...
  AICc.value <- Inf
  res <- try(gw_reg_all(X, Y, dp.locat, di, FALSE, dp.locat, DM.given, dMat, TRUE, p, theta, longlat, bw, kernel, adaptive))
  if(!inherits(res, "try-error")) {
    betas <- res$betas
    s_hat <- res$s_hat
    AICc.value <- AICc1(Y, X, betas, s_hat, di)
  } else AICc.value <- Inf 
...
gwr.bic<-function(bw, X, Y, kernel="bisquare",adaptive=FALSE, dp.locat, p=2, theta=0, longlat=F,dMat, verbose=T,parallel.method=F,parallel.arg=NULL)
{
  ...
  if (parallel.method == FALSE) {
    ...
    if(!inherits(res, "try-error")) {
      betas <- res$betas
      s_hat <- res$s_hat
      BIC.value <- gw_BIC(Y, X, betas, s_hat, di)
    }
    else BIC.value <- Inf
  } else if (parallel.method == "omp") {
    if (missing(parallel.arg)) { threads <- 0 } else {
